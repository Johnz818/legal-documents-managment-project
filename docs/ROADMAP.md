...
- The Phase 5 deterministic document-generation journey is complete; browser
  DOCX editing, persisted drafts/revisions/finalization, OCR, AI assistance,
  and system-wide timestamp migration remain separately scoped future work.
...
CI3 is a reserved future ticket rather than the next delivery step. Resume it
after P1 selects the deployment platform and container registry. Before image
publishing is implemented, approve:

- registry ownership and image names;
- immutable release-tag and source-commit conventions;
- the deployment consumer and supported CPU architectures;
- registry credential and secret-management boundaries;
- image retention and cleanup policy;
- whether provenance, SBOM generation, and image signing are required.
...
| CI2 — Container verification (complete) | Build backend and frontend container images on pull requests without publishing. | `Container / Backend` and `Container / Frontend` pass on GitHub without publishing packages. | `ci: verify container images` |
| CI3 — Image publishing (deferred until P1) | Publish immutable, versioned images only after the deployment platform, registry, naming, tagging, credential, retention, architecture, and supply-chain requirements are approved. | Dry run and test release with traceable immutable tags; confirm the selected deployment consumer can pull the images. | `ci: publish release images` |
...
| G4 — Document generation (complete) | Prepare Case/system suggestions in a client-supplied IANA timezone, accept a complete human-reviewed value set, render and store the completed DOCX, and persist successful generation traceability with stable relational Generation Value records. Use request idempotency, immutable-template integrity verification, exclusive storage objects, post-rollback race classification, and best-effort compensation. | MySQL, storage-contract, service, and API tests cover preparation, timezone-aware source validation and overrides, scalar/date contracts, retained exact values, idempotent replay and races, template-content integrity, storage/persistence failures, and compensation; the representative Postman suite and generated Chinese DOCX inspection passed. | `feat: generate case documents` |
| G5 — Frontend integration (complete) | Replace the relevant template-management and generation mock flows with live template inspection/publication/version management, input preparation, browser-detected IANA timezone, human value review, idempotent generation, derived output availability, generated-document download, and generated-event time displayed in the browser timezone. | Frontend tests, backend timestamp/query tests, full checks/builds, and manual template-management, selection, timezone/date, input, stale-value resolution, generation, exact retry, removed-output, download, and error checks pass. | `feat: integrate document generation` and focused live-template-management commit |
